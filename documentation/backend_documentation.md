...
| `/api/secret-keys` | CRUD kluczy API użytkownika (SecretKey). Endpoint `GET /transport-key` udostępnia klucz AES-256 do szyfrowania transportowego. Dodawanie: klucz przesyłany jako AES-256-GCM ciphertext, dekodowany w routerze, następnie szyfrowany Fernet at rest. Usuwanie, walidacja via OpenRouter API. Filtrowane po `user_id`. |
...
- Logika szyfrująca dla kluczy API (`app/encryption.py`): AES-256-GCM do szyfrowania transportowego (klucz w `DATA_DIR/transport_aes.key`), Fernet do szyfrowania at rest (klucz w `DATA_DIR/fernet.key`). Frontend szyfruje klucz API za pomocą Web Crypto API, backend odszyfrowuje AES i reszyfruje Fernet przed zapisem.
