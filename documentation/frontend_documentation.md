...
- Edytor prototypu (`/generate/[id]/editor`) zapisuje zmiany przyciskiem **„Zapisz wersję roboczą”** do tabeli `prototypes` (`PUT /api/prototypes/{generation_id}`), dzięki czemu materiał można później kontynuować i finalizować.- **Persystencja komentarzy:** Przy każdym zapisie (zarówno wersji roboczej w edytorze, jak i sfinalizowanego dokumentu) helper `extractCommentsFromHtml()` wyodrębnia z HTML wszystkie znaczniki `<mark class="tiptap-comment" data-comment="...">` i wysyła je jako strukturyzowany JSON (`comments_json`) obok treści HTML. Backend przechowuje ten JSON w kolumnie `prototypes.comments_json`. Komentarze przetrwają finalizację, przeniesienie do wersji roboczej, kopiowanie dokumentu/draftu oraz eksport do DOCX (treść tekstu jest zachowana, a anotacje komentarzy dostępne w bazie).
### `layout/`
