...
- **Bubble menu — wzajemne wykluczanie tabeli i komentarzy:** `TableBubbleMenu` wyświetla się wyłącznie gdy kursor/zaznaczenie znajduje się wewnątrz komórki tabeli (`editor.isActive("tableCell")` / `"tableHeader"` lub `CellSelection`). `CommentBubbleMenu` stosuje podwójną ochronę — najpierw sprawdza `editor.isActive("tableCell"/"tableHeader")`, a następnie wykonuje ręczne przejście po drzewie węzłów (`$from.node(d)`) — i nigdy nie pojawia się, gdy kursor jest wewnątrz tabeli.
- **Usuwanie komentarzy:** Przycisk „Usuń komentarz" w `CommentBubbleMenu` korzysta z `extendMarkRange("comment")` przed `unsetMark("comment")`, dzięki czemu komentarz jest usuwany poprawnie nawet gdy kursor jest ustawiony wewnątrz komentarza bez zaznaczenia tekstu.
- **`RepromptInput.tsx`:** pole AI jest renderowane jako `position: fixed` i jest stale przyklejone 50 px od dolnej krawędzi ekranu (`bottom: 50px`) dla wszystkich urządzeń. Komponent nie obsługuje przeciągania ani zapisu pozycji.
...
- **Persystencja komentarzy:** Przy każdym zapisie (zarówno wersji roboczej w edytorze, jak i sfinalizowanego dokumentu) helper `extractCommentsFromHtml()` wyodrębnia z HTML wszystkie znaczniki `<mark class="tiptap-comment" data-comment="...">` i wysyła je jako strukturyzowany JSON (`comments_json`) obok treści HTML. Backend przechowuje ten JSON w kolumnie `prototypes.comments_json`. Komentarze przetrwają finalizację, przeniesienie do wersji roboczej, kopiowanie dokumentu/draftu oraz eksport do DOCX (treść tekstu jest zachowana, a anotacje komentarzy dostępne w bazie).
### `layout/`
