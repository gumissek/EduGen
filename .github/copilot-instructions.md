...
- Use the `ThemeProvider` with `createTheme` for consistent design tokens (colors, typography, spacing) across the application
- Use the `sx` prop for one-off style overrides — prefer it over inline `style` or creating new styled components for simple cases
- Leverage MUI's `useTheme` hook to access theme values inside component logic and custom hooks
- Prefer MUI component props (`variant`, `color`, `size`, `component`) over custom styling where the API covers the use case
- Use `CssBaseline` in the root layout for consistent cross-browser baseline styles
- Implement responsive design with MUI breakpoint utilities: `sx={{ display: { xs: 'none', md: 'block' } }}` and `useMediaQuery`
- Use `Grid` and `Stack` for layout composition; avoid manual flexbox where MUI layout components suffice
- Extend components with Emotion's `styled()` from `@emotion/styled` when encapsulating reusable styled variants (not one-off overrides)
- Support Dark/Light mode via `ColorModeContext` — never hardcode colors; always reference theme tokens
- Use `alpha()` from `@mui/material/styles` for transparent color variants derived from theme palette
